...
- A server error (5xx) from Spotify's token endpoint (`accounts.spotify.com`) is now treated as a temporary outage like any other Spotify 5xx: one clear error line and a 30 second refresh backoff, instead of the misleading `Token request for spotcast failed (unknown): unknown_error` followed by a raw `503, message='Service Unavailable'` error on every cycle. Genuine token failures (4xx) now include the HTTP status in the log.
- The device manager applies the same 30 second backoff after a failed device refresh: it previously set the retry supervisor's internal health flag directly, which bypassed both the backoff scheduling and the error de-duplication reset on recovery.
